  ...
  on_supported_os.each do |os, os_facts|
    context "on #{os}" do
      ...
      context 'with pam' do
        ...
        it { is_expected.to contain_file("#{server_directory}/test_server.conf").with_content(%r{^plugin #{pam_module_path} "?login"?$}) }
        ...
        context 'with pam_module_arguments' do
          ...
          it { is_expected.to compile.with_all_deps }
          it { is_expected.to contain_file("#{server_directory}/test_server.conf").with_content(%r{^plugin #{pam_module_path} "openvpn login USERNAME password PASSWORD"$}) }
        end
      end
